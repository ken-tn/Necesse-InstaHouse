...
public class InstaHousePreset extends Preset {
    ...
    public InstaHousePreset() {
        super(DefaultHousePreset);
    }

    public void ApplyUpdate(int permissionLevel, String script) {
        ...
        if (InstaHouseMod.SettingsGetter.getBoolean("client_preset_allowed")) {
            ...
            if (InstaHouseMod.SettingsGetter.getBoolean("moderators_only")) {
                if (permissionLevel >= PermissionLevel.MODERATOR.getLevel()) {
                    this.clearPreset();
                    this.applySave(0, 0, getLoadDataFromScript(script));
                }
            } else {
                this.clearPreset();
                this.applySave(0, 0, getLoadDataFromScript(script));
